...
const BalanceAdjustmentsView = ({ isOpen, onClose, data }) => {
  const { id } = useParams();
  const { data: viewData } = useGetBalanceAdjustmentsByIdQuery(id);
  ...
  return (
    <div>
      <ViewModal
        ...
        onClose={onClose}
        data={data}
        title="Balance Adjustments Details"
        items={items}
      />
    </div>
  );
...
export default BalanceAdjustmentsView;
