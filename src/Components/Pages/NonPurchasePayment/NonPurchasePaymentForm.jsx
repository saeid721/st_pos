...
import { useCreateNonPurchasePaymentsMutation, useGetNonPurchasePaymentSummaryBySupplierQuery, useUpdateNonPurchasePaymentsMutation } from "../../../store/api/app/NonPurchasePayment/nonPurchasePaymentApiSlice";

const NonPurchasePaymentForm = ({ id, data }) => {
  ...
  const {
    ...
    id,
    id ? useUpdateNonPurchasePaymentsMutation : useCreateNonPurchasePaymentsMutation
  );
  ...
  const { data: nonPurchasePaymentSuppliersData } = useGetNonPurchasePaymentSummaryBySupplierQuery({
    supplier_id: supplier_id,
    branch_id: branchId,
  })

  console.log("nonPurchasePaymentSuppliersData::", nonPurchasePaymentSuppliersData);
            ...
            supplier_id && (
              <>
                <div className="grid grid-cols-3 gap-5">
                  <TextInput
                    name="invoice_total"
                    label="Non Purchase Total"
                    type="text"
                    register={register}
                    placeholder="Enter Purchase Total"
                    value={nonPurchasePaymentSuppliersData?.data?.total_non_purchase_amount}
                    defaultValue={nonPurchasePaymentSuppliersData?.data?.total_non_purchase_amount}
                    disabled={true}
                  />
                  <TextInput
                    name="total_paid"
                    label="Non Purchase Paid"
                    type="text"
                    register={register}
                    placeholder="Enter Purchase Paid"
                    value={nonPurchasePaymentSuppliersData?.data?.total_paid_amount}
                    defaultValue={nonPurchasePaymentSuppliersData?.data?.total_paid_amount}
                    disabled={true}
                  />
                  <TextInput
                    name="total_due"
                    ...
                    value={nonPurchasePaymentSuppliersData?.data?.total_due_amount}
                    defaultValue={nonPurchasePaymentSuppliersData?.data?.total_due_amount}
                    disabled={true}
                  />
                </div>
...
export default NonPurchasePaymentForm;
