...
const PurchaseReturnTableBody = ({
  ...
  const purchaseReturnProduct = purchaseProductsData?.find(
    (product) => product?.product_id === item?.product?.id
  );
  console.log("purchaseReturnProduct", purchaseReturnProduct);
    ...
    <tr key={item?.id} className="hover:bg-gray-50">
      ...
      <td className="py-3 px-4">
        <div className="flex items-center space-x-2">
          ...
          <span className="w-8 text-center">{purchaseReturnProduct?.quantity}</span>
          <button
            // onClick={() => updateQuantity(index, 1)}
            onClick={(e) => {
              e.stopPropagation();
      ...
      {/* Return Price */}
      <td className="py-3 px-4 text-sm text-gray-900">{(item?.unit_cost * purchaseReturnProduct?.quantity)?.toFixed(2)}</td>
    </tr>
  );
};

export default PurchaseReturnTableBody;
